...
*/
var Sticky = function Sticky(configuration) {
  ...
  if (this.config.heightElements) {
    this.heightElements = [];
    for (var elem = 0; elem < this.config.heightElements.length; elem += 1) {
...
Sticky.prototype.scroll = function handleScrollEvents() {
  ...
  if (this.config.lowestElement) {
    var lowestTop = this.lowestElement.getBoundingClientRect().top;
...
Sticky.prototype.setFillHeight = function calcaulateHowMuchPaddingToAddToTheWrapper() {
  if (this.heightElements) {
    var height = 0;
    for (var elem = 0; elem < this.heightElements.length; elem += 1) {
...
Sticky.prototype.resize = function handleResizeEvents() {
  ...
  this.setFillHeight();
  var computedWidth = this.element.parentNode.clientWidth + 'px';
  if (!this.width) {
...
Sticky.prototype.setTop = function setTheTopValue() {
  ...
  var scrollTop = this.scrollTop();
  var docTop = elemOffset.top + scrollTop;
